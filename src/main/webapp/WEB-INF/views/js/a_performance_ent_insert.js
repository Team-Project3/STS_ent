	...
	$("#sdatePicker").datepicker(
			{
				...
				,
				dayNamesMin : [ '일', '월', '화', '수', '목', '금', '토' ] // 달력의 요일 부분
																	// 텍스트
				,
...
function a_insertperformance() {
	...
	if (selectValue == 1) {
		...
		if (r > 0 && a > 0 && b > 0) {
			if (r > 50 || a > 50 || b > 50) {
				alert("인원수는 최대 50명까지 설정 가능합니다.");
				return false;
			}
			var cseatElements = document.getElementsByClassName("cseat");
			for (var i = 0; i < cseatElements.length; i++) {
				var inputValue = cseatElements[i].value;
		...
		if (width > 0 && length > 0) {
			...
			if (width > 9 || length > 9) {
				alert("가로,세로 최대 9칸 까지 설정가능합니다.");
				return false;
			}
			
			var tseatElements = document.getElementsByClassName("tseat");
			for (var i = 0; i < tseatElements.length; i++) {
				var inputValue = tseatElements[i].value;
				if (/\D/.test(inputValue) || /\./.test(inputValue)) {
					alert("좌석 정보가 올바르지 않습니다.");
					return false;
				}
		...
		if (am > 0 && pm > 0) {
			...
			if (am > 40 || pm > 40) {
				...
				alert("오전,오후 최대 40명씩 설정 가능합니다.");
				return false;
			} 
			
			var mseatElements = document.getElementsByClassName("mseat");
			for (var i = 0; i < mseatElements.length; i++) {
				var inputValue = mseatElements[i].value;
				if (/\D/.test(inputValue) || /\./.test(inputValue)) {
					alert("좌석 정보가 올바르지 않습니다.");
					return false;
				}
	...
	if (document.getElementById("price").value < 1000
			|| document.getElementById("price").value % 1000 !== 0) {
		alert("공연가격을 1000단위로 해주세요");
		return false;
	}
	if (/\D/.test(document.getElementById("price").value)
			|| /\./.test(document.getElementById("price").value)) {
