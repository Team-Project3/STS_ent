...
<body class="sb-nav-fixed">
	...
	<div id="layoutSidenav_content">
		...
		<div class="container-fluid px-4">
			<h1 class="mt-4">예약내역 목록</h1>
			<ol class="breadcrumb mb-4">
				<li class="breadcrumb-item active">Booking List</li>
			</ol>
			<div class="card mb-4">
				<div class="card-header">
					<i class="fas fa-table me-1"></i> 전체 예약 목록
				</div>
				<div class="card-body">
					<table id="datatablesSimple">
						...
						<tbody>
							<c:forEach var="book" items="${booklist}" varStatus="loop">
								<tr>
									<td><a id="detail"
										href="a_performance_booking_detail?bseq=${book.bseq}">${book.bseq}</a></td>
									<td><a id="detail"
										href="a_performance_booking_detail?bseq=${book.bseq}">${book.id}</a></td>
									<td><c:choose>
									...
									<td>${book.tname}</td>
									<td><fmt:formatDate value="${book.dday}"
											pattern="yyyy-MM-dd" var="dday" /> ${dday}</td>
									<td>${book.head}</td>
									<td>${book.seat}</td>
									<td><c:set var="totalprice"
											value="${book.head*book.price}"></c:set>${totalprice}</td>
									<td>${book.bstatus}</td>
								</tr>
							</c:forEach>
						</tbody>
